...
export const login = async (dispatch, user) => {
  ...
  try {
    ...
    dispatch(
      loginFailure(error?.response?.data?.msg || "Something went wrong")
    );
  }
};
...
export const getAllProducts = async (
  ...
  try {
    const response = await publicRequest.get(
      `/products/all?page=${page || 1}&category=${
        category ? category : ""
      }&color=${color ? color.toLowerCase() : ""}&size=${
        size ? size : ""
      }&sort=${sort ? (sort === "newest" ? "-createdAt" : sort) : ""}`
    );
    dispatch(api.getProducts(response?.data));
  } catch (error) {
    dispatch(
      api.fetchingError(error?.response?.data?.msg || "Something went wrong")
    );
  }
};
